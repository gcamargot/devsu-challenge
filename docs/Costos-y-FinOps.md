...
![Analisis de costos en Azure Cost Management, agrupado por servicio, ubicacion y resource group](evidencia-17-finops.png)

Azure Cost Management sobre la suscripcion (Cost analysis), costo acumulado y desglosado. El costo real ronda los 11 USD en la ventana, dominado por Azure Grafana Service (6 USD) y las VMs del node pool (4 USD), lo que confirma lo dicho arriba: la observabilidad administrada es la pieza mas cara y por eso es lo primero que se apaga. Los donuts agrupan el gasto por servicio, por ubicacion (casi todo en `us east 2`) y por resource group (el grueso en `devsu-rg` y su MC_ del cluster). El desglose por recurso se cruza con las etiquetas de costeo (`project`, `owner`) que lleva cada recurso (`az resource list -g devsu-rg --query "[].tags"`); el budget esta definido como IaC en [finops.tf](https://github.com/gcamargot/devsu-challenge/blob/master/terraform/finops.tf), gateado en `budget_contact_emails` (se activa cargando un email de contacto).
