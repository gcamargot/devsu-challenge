...
> <font color="#0969da">**Nota:**</font> varias de estas mejoras ya estan parcialmente escritas como IaC y van detras de un flag apagado (la VNet propia con PostgreSQL privado), o ya existen en una version inicial que solo falta promover (la firma de imagenes ya esta de punta a punta: el CI firma, el CD copia la firma a ACR y la policy Kyverno de verificacion corre en modo Audit reportando PASS; lo unico pendiente es el cutover a Enforce). Cuando es el caso lo decimos explicitamente, asi se distingue lo que es empezar de cero de lo que es apretar un interruptor.
...
Esta mejora es de las que ya estan a un paso, porque el grueso ya esta en produccion. En CI las imagenes se firman con cosign keyless (Fulcio emite un certificado efimero atado a la identidad OIDC del workflow y la firma queda en el log de transparencia Rekor; ver [.github/workflows/ci.yml](https://github.com/gcamargot/devsu-challenge/blob/master/.github/workflows/ci.yml)). Y en el CD, despues del `az acr import` (que copia solo la imagen, no su firma), un `cosign copy` lleva la firma y las attestations de GHCR a ACR, asi la imagen que termina corriendo en el cluster queda firmada igual que en GHCR (ver [.github/workflows/cd.yml](https://github.com/gcamargot/devsu-challenge/blob/master/.github/workflows/cd.yml)). Del lado del cluster corre una policy Kyverno `verifyImages` que valida esa firma, en [k8s/policies/verify-images.yaml](https://github.com/gcamargot/devsu-challenge/blob/master/k8s/policies/verify-images.yaml), acotada solo a las imagenes de `devsu-challenge` (las publicas como postgres, ingress-nginx, cert-manager o kyverno nunca se matchean ni se bloquean), y con su propia credencial de pull del ACR (el secret `acr-pull`, ver [k8s/policies/acr-pull-secret.md](https://github.com/gcamargot/devsu-challenge/blob/master/k8s/policies/acr-pull-secret.md)). Hoy esa policy esta en modo Audit, pero ya no es un placebo: verifica de verdad la imagen real que corre y reporta PASS ("image verified") en el policyreport del namespace. Es decir, firmar y verificar ya no es lo pendiente: eso esta hecho. Lo unico que falta es el cutover de Audit a Enforce, que consiste en pasar `validationFailureAction` a Enforce de la mano de `verifyDigest: true` y `mutateDigest: true` (en Audit se deja `verifyDigest: false` porque las imagenes se despliegan por tag y Kyverno en audit no puede mutar a digest, asi que exigir digest da "missing digest" aun con la firma valida). Con el flip, el cluster pasa a rechazar cualquier imagen propia que no este firmada por nuestro pipeline.

> <font color="#1a7f37">**Tip:**</font> el orden importa, y la primera mitad ya esta cumplida: la imagen firmada ya rueda en produccion y los reportes de Audit confirman que matchea la firma esperada (PASS). Recien con eso verificado se flipea a Enforce, sumando `verifyDigest: true` + `mutateDigest: true`. Hacerlo al reves deja la app sin poder arrancar.
